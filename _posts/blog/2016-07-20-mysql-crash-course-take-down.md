...
**外键(foreigh key)**：外键为某个表中的一列，它包含另一个表的主键值，定义了两个表之间的关系。
...
**外部联结**：包含了那些在相关表中没有关联行的行。(`RIGHT`指出的是`OUTER JOIN`右边的表，而`LEFT`指出的是`OUTER JOIN`左边的表)
...
**触发器**是MySQL响应一下任意语句而自动执行的一条MySQL语句(或位于`BEGIN`和`END`语句之间的一组语句)。
